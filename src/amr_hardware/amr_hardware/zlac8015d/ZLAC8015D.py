...
class Controller:
	...
	def __init__(self, port="/dev/ttyUSB0"):
		...
		self.client = ModbusClient(method='rtu', port=self._port, baudrate=115200, timeout=0.05)
	...
	def get_wheels_tick(self):
		# Single attempt — tidak pakai modbus_fail_read_handler supaya tidak infinite loop
		result = self.client.read_holding_registers(self.L_FB_POS_HI, 4, unit=self.ID)
