# --- CHANGE 1: Use the modern 3.x client import path and explicit RTU Framer ---
...
class Controller:
    ...
    def __init__(self, port="/dev/ttyUSB0"):
        ...
        # --- CHANGE 2: Added framer and explicit keyword parameters ---
    ...
    # --- CHANGE 3: Swapped unit= to unit= in all functions below ---

    def modbus_fail_read_handler(self, ADDR, WORD):
        read_success = False
        reg = [None]*WORD
        while not read_success:
            result = self.client.read_holding_registers(ADDR, WORD, device_id=self.ID)
            try:
                for i in range(WORD):
    ...
    def set_mode(self, MODE):
        ...
        result = self.client.write_register(self.OPR_MODE, MODE, device_id=self.ID)
        return result
    ...
    def enable_motor(self):
        result = self.client.write_register(self.CONTROL_REG, self.ENABLE, device_id=self.ID)

    def disable_motor(self):
        result = self.client.write_register(self.CONTROL_REG, self.DOWN_TIME, device_id=self.ID)

    def get_fault_code(self):
        fault_codes = self.client.read_holding_registers(self.L_FAULT, 2, device_id=self.ID)
    ...
    def clear_alarm(self):
        result = self.client.write_register(self.CONTROL_REG, self.ALRM_CLR, device_id=self.ID)
    ...
    def move_left_wheel(self):
        result = self.client.write_register(self.CONTROL_REG, self.POS_L_START, device_id=self.ID)

    def move_right_wheel(self):
        result = self.client.write_register(self.CONTROL_REG, self.POS_R_START, device_id=self.ID)
    ...
    def get_wheels_tick(self):
        result = self.client.read_holding_registers(self.L_FB_POS_HI, 4, device_id=self.ID)
        if not hasattr(result, 'registers'):
            raise Exception(f"Modbus read failed: {result}")
